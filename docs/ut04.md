...
El producto que utilizan los materiales como ejemplo es Odoo (y su antecesor OpenERP); su base de datos se administra con pgAdmin. Conviene separar el concepto general —lo que trabaja esta unidad: analizar la empresa, seleccionar módulos, adaptar tablas y vistas, crear consultas, formularios, informes y paneles, integrar con otros sistemas— de los detalles concretos de esa herramienta, que aquí sirven de ilustración y no de norma. La carpeta de esta unidad combina en su nombre restos del título de la unidad anterior ("Consulta de la información") con el propio de esta ("Implantación de sistemas ERP-CRM en una empresa").

## ¿Qué vamos a aprender?

Esta unidad trabaja **adaptar sistemas ERP-CRM**: identificar los requerimientos de un supuesto empresarial y cubrirlos con las herramientas que el propio sistema proporciona, sin necesidad (en esta unidad) de programar módulos nuevos desde cero.
...
### El equipo de implantación y las etapas del proyecto

Toda etapa de la implantación debe estar detallada indicando cuándo se realiza, quién es el responsable y a qué áreas afecta. En el proceso se distinguen varias **figuras clave**: la **dirección o responsables de la empresa**, que toman las decisiones y deben estar plenamente implicados; el **jefe de proyecto**, interno o externo, que valida, verifica y hace de interlocutor entre todos los miembros del equipo; el **responsable de la migración de datos**, que conoce bien el sistema antiguo y las necesidades del nuevo; y el **equipo de consultoría**, encargado del análisis inicial de procesos y requisitos, la propuesta de solución, la instalación y configuración, la formación de las personas usuarias y la programación a medida de los módulos necesarios.

Dentro de las cuatro grandes fases, las **etapas más destacadas** son: análisis de procesos y enfoque de la solución (qué necesidades cubre la aplicación, con qué módulos y qué queda para desarrollo a medida); planificación del proyecto con estimación de tiempos y coste; fase de instalación (incluye el traspaso de datos, el inicio de la programación a medida y la formación a responsables de área); fase de consultoría, formación a usuarios e instalación de los módulos a medida; fase de pruebas manteniendo el sistema antiguo en paralelo; puesta en marcha; revisión de funcionalidades y ajustes; y finalización del proyecto.

Pasado un tiempo prudencial desde la puesta en marcha se hace la **revisión final**, que comprueba si se han alcanzado los objetivos previstos, si los módulos funcionan de forma adecuada, si las personas usuarias están suficientemente formadas, si se ha cumplido el presupuesto y si no hay errores, sobrecargas ni paradas no programadas. Los **factores de éxito** de una implantación son una buena dirección del proyecto, una dotación de medios adecuada y la implicación y el compromiso de toda la organización; su ausencia es también la primera de las causas de fracaso.
...
- Figuras clave del proyecto: dirección, jefe de proyecto, responsable de migración de datos y equipo de consultoría. Factores de éxito: buena dirección, medios adecuados e implicación de toda la organización.
...
El material describe la construcción de un tablero **desde el asistente de tableros**:
*Informes → Configuración → Crear tablero*. Cada entrada del tablero se asocia con una
**acción** distinta (un informe) y se ajustan sus características de presentación; se añaden
tantos componentes como se necesiten. Una vez definido el tablero, se insertan en él los
distintos informes.

La propia interfaz de Odoo ofrece además una segunda vía: el mensaje del **tablero personal
vacío** indica que se añaden informes con la opción **"Añadir al tablero"** disponible en
cualquier listado en vista de lista o de gráfico, aplicando antes los **filtros** y
**agrupaciones** que interesen para que el componente muestre ya esos datos.
...
*Figura: el tablero personal parte vacío; la propia pantalla explica cómo rellenarlo desde los listados. Origen: ApuntesU4SGE.pdf.*
...
- Vía documentada: el asistente *Informes → Configuración → Crear tablero* (asociando **acciones**/informes). La interfaz de Odoo añade la opción **"Añadir al tablero"** desde un listado en vista lista o gráfico, tras aplicar **filtros** y **agrupaciones**.
...
Para **exportar** desde el ERP de ejemplo se selecciona la vista y los registros y se usa el enlace **Exportar**, que abre un diálogo de selección de campos y su orden. El material lo sitúa en el *menú lateral derecho* del cliente web; en las versiones actuales de la interfaz es la opción *Exportar* del menú **Más** (o de acciones). Es importante elegir el tipo **"compatible con importación"** si los datos van a volver a un sistema del mismo tipo. El resultado se guarda como fichero **CSV**.
...
La convivencia de sistemas y el volcado de datos hacen imprescindible **programar copias de seguridad** de las bases de datos. En el ERP de ejemplo esto exige un módulo aparte: al instalar `auto_backup` se crea una acción planificada (*Backup scheduler*) que hay que marcar como activa e indicando la hora, la periodicidad y si se recuperan las copias no realizadas; después, en *Configuración → Personalización → Configure Backup*, se añade un registro por cada base de datos (nombre exacto, servidor y carpeta de destino).
...
11. Con el asistente *Informes → Configuración → Crear tablero*, asociando cada entrada a una acción/informe; la interfaz de Odoo permite además "Añadir al tablero" desde un listado en vista lista o gráfico, tras filtrar y agrupar (sección 7).
