...
10. [Fuentes (documentos de la carpeta)](#fuentes-documentos-de-la-carpeta)
...
En términos llanos, superar este resultado supone ser capaz de leer la documentación de un ERP-CRM, entender qué piezas lo componen y dejarlo instalado y en marcha de varias formas distintas, ajustándolo a lo que pide una empresa. Los criterios de evaluación asociados se pueden parafrasear así:
...
- `DAM_SGE02_Instalación y configuración de sistemas ERP-CRM.pdf` — Documento principal de la unidad. Tipos de licencia y software libre en el mercado ERP; ERP libres (Openbravo, Odoo, Dolibarr); tareas de instalación e implantación; tipos de instalación (máquina virtual, paquetes, personalizada, en línea) y monopuesto frente a cliente/servidor; procesos de instalación y servicios de acceso; parámetros de configuración y ficheros de Odoo 13 (`odoo.conf`, `pg_hba.conf`, puerto 8069); actualización entre versiones; tipos de módulos, módulo base y módulos funcionales (contable, compras/ventas/almacén, facturación, personal, CRM); instalación de módulos y localización del país; asistencia técnica remota con VNC; herramientas de programación. Origen de la mayoría de las capturas de Odoo y Synaptic.
- `ApuntesU2SGE.pdf` — Apuntes breves. Tipos de licencia (permisivas, robustas, código cerrado; freeware/payware/shareware); tipos de instalación (monopuesto, cliente-servidor, cliente-servidor web, SaaS); estructura en capas de OpenERP y patrón MVC; requerimientos hardware/software; y definición de los entornos de desarrollo, pruebas y explotación. Origen del diagrama "MVC de OpenERP".
- `Instalación y configuración de un ERP - P1.pdf` — Contexto histórico de los ERP-CRM; sistemas en la propia empresa frente a la nube (ventajas, inconvenientes, factores de decisión, RGPD/LOPDGDD); tipos de instalación (carpeta compartida, on-premise, nube) y modelos IaaS/PaaS/SaaS; licencias libres frente a propietarias; preparación del servidor (hardware, seguridad, SAI, RAID/ZFS/BTRFS, virtualización con hipervisor o contenedores, Proxmox); requisitos de Odoo 17.
- `Instalación y configuración de un ERP - P2.pdf` — Instalación de Odoo 17: manual en Ubuntu Server, con contenedores Docker (PostgreSQL 15 + Odoo 17) y con Docker Compose (`docker-compose.yml`, `docker compose up -d`/`down`); diferencias entre contenedor de producción y de desarrollo (volúmenes, `--dev=all`); puesta en marcha y configuración inicial de Odoo 17 con el Password maestro. Origen de las capturas del gestor de aplicaciones y de la configuración inicial de Odoo 17.
- `ActObligatoriaU2SGE.docx` (convertido a PDF para su lectura) — Actividad práctica: instalación de OpenERP 7.0 en Ubuntu y configuración para una PYME española (creación de base de datos; módulos de CRM, almacén, compras, ventas y contabilidad; módulos de localización `l10n_es_*`; ejercicio fiscal y periodos; diarios y cuentas 570000/572000; usuarios; categorías de producto; copia de seguridad). Se usa como ejemplo de en qué consiste configurar módulos en un supuesto real.
