...
12. [Fuentes (documentos de la carpeta)](#fuentes-documentos-de-la-carpeta)
...
Un apunte sobre el nombre: la carpeta de material se titula «Identificación de sistemas ERP-CRM-**BI**», mientras que en la programación provisional la unidad figura como «Identificación de sistemas ERP-CRM» y su lista de Contenidos no incluye una viñeta propia de *Business Intelligence*. El material sí trata el BI como parte natural de estos sistemas, de modo que aquí se explica dentro de los apartados de concepto, pero el esqueleto del resumen sigue las viñetas de la programación.
...
En términos llanos, superar este resultado supone lo siguiente:
...
El criterio d) del RA1 pide identificar el sistema operativo adecuado a cada ERP-CRM. El material aporta como guía que **la mayoría de los proveedores han optado por Windows** para desarrollar sus ERP, mientras que **Linux** se ha ido potenciando, sobre todo en grandes empresas. En el escenario práctico se cubren las dos familias (Windows y Ubuntu), y productos como OpenBravo (interfaz web) u Odoo (servidor Python) admiten despliegue en ambas.
...
El criterio f) del RA1 exige **verificar las configuraciones del sistema operativo y del gestor de datos** para garantizar que el ERP-CRM funciona. El material trata este punto de forma escueta y sobre todo de manera implícita, a partir del escenario práctico; a continuación se ordena lo que aporta.
...
Los criterios g) y h) del RA1 piden **documentar las operaciones realizadas** y **documentar las incidencias producidas durante el proceso**. El material de esta unidad menciona la documentación como requisito, pero no incluye una plantilla ni un procedimiento detallado; lo que sigue reúne y ordena su sentido.
...
- **Incidencias y su resolución:** errores encontrados durante el proceso, causa identificada, solución aplicada y, si procede, medidas para evitar que se repitan. Este registro es lo que pide expresamente el criterio h).
...
## Fuentes (documentos de la carpeta)

- `T01. Identificación de sistemas ERP-CRM-BI (1).pdf` — texto base más completo y actual: conceptos de ERP, CRM, BI y ERP-CRM-BI, revisión de productos (SAP, Oracle, Microsoft, OpenBravo, Odoo), ventajas e inconvenientes, programar frente a personalizar, licencias, retos de implantación y glosario empresarial.
- `2 DAM SGE UD 1 Sistemas ERP-CRM-BI.pdf` — evolución de la informática de gestión, historia MRP → ERP, características de ERP y de CRM, revisión de CRM actuales, arquitectura ERP-CRM (base de datos relacional y cliente-servidor), datos que pide toda instalación, listado de ERP libres y privativos.
- `ApuntesU1SGE.pdf` — conceptos de gestión empresarial, concepto de ERP y CRM, revisión de ERP, tabla comparativa OpenBravo / OpenERP, factores para elegir un ERP, ventajas y desventajas, y escenario práctico de instalación (OpenERP + PostgreSQL en Windows 7 y Ubuntu).
- `Introduccion_a_sistemas_erp-crm.pdf` — presentación con objetivos y características de un ERP, ejemplo de circuito de un pedido, fases de implementación, modelos on-premise y cloud (IaaS, PaaS, SaaS) con sus beneficios e inconvenientes, y ficha de Odoo (ediciones Community y Enterprise).
- `1_ERP_CRM_BI.pptx.pdf` — presentación de síntesis: definiciones de ERP, CRM y BI, arquitectura modular, características clave, principales proveedores propietarios y de código abierto, ventajas, inconvenientes y retos de la implantación.
